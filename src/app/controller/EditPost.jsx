      ...
      {url ? (
        <>
          <img
            src={url}
            alt={"image-from" + url}
            className="figure-img img-fluid d-block mx-auto"
          ></img>
        </>
      ) : (
        <></>
...
function EditPosts(props) {
      ...
      .once("value", function (data) {
        if (data.val()) {
          ...
          if (_data.image) {
            ...
            setMedia(
              decodeURIComponent(
                _data.image.split("?")[0].split(`${storageName}/o/`)[1]
              )
            );
          }
          if (_data.message) setInput(_data.message.trim());
          setPostdata(_data);
          document.querySelector(`#text-input${props.id}`).value =
            _data.message;
        }
      });

    if (
      document.querySelector(`#text-input${props.id}`).value.trim() !== input
    ) {
      setInput(document.querySelector(`#text-input${props.id}`).value.trim());
    }

    document
      .querySelector(`#editPostModal${props.id}`)
  ...
  function handleFile(selectorFile) {
    ...
    if (supportImageType.includes(file.type)) {
      ...
      document.querySelector("#blob" + props.id).innerHTML =
        "We doesn't support this file type!";
    }
  }
  ...
  function complete() {
    firebase
      ...
      .then((url) => {
        let capt =
          document.querySelector(`#text-input${props.id}`).value.trim() !==
          input
            ? document.querySelector(`#text-input${props.id}`).value.trim()
            : input;
        postDb
          ...
          .then(async () => {
            try {
              await firebase.storage().ref(media).delete();
            } catch (err) {
              console.log(err);
            } finally {
              redirect("/");
  ...
  function postCapt() {
    ...
    if (img !== "") {
      ...
      if (
        (capt === postdata.message && blob === postdata.image) ||
        (!postdata.message && !capt && blob === postdata.image)
      ) {
        redirect("/");
      } else {
        let task = firebase
