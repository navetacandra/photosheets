...
function Posts() {
  const user =
    JSON.parse(localStorage.getItem("user")) || firebase.auth().currentUser;
  ...
  function complete() {
    firebase
      ...
      .then((url) => {
        let capt =
          document.querySelector(`#text-input-post`).value.trim() !== input
            ? document.querySelector(`#text-input-post`).value.trim()
            : input;
        postDb
          .push({
            image: url,
